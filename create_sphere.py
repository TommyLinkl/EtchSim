...
def create_sphere():
    if len(sys.argv) != 2:
        print("Usage: python create_sphere.py CALC_DIR/")
        return
    
    calc_setting['calc_dir'] = sys.argv[1]
    load_input_from_json(f"{sys.argv[1]}input.json")

    # initialize_wz_lattice -> sphere
    buffer = npl_params['buffer']      # all in AA
    sphere_radius = npl_params["sphere_radius"]
    wz_lattice = initialize_wz_lattice(sphere_radius*2+buffer, sphere_radius*2+buffer, sim_params, verbosity=calc_setting['verbosity'])
    

    # initialize_hex_NPL -> sphere
    start_time = time.time()
    for site in wz_lattice:
        if NPL_within_sphere(site.real_space_coord, sphere_radius):
            site.has_atom = True
